...
public class ActionOnTradePower extends Power {
    ...
    public static PowerFactory<?> getFactory() {
        return new PowerFactory<>(Appli.identifier("action_on_trade"),
                ...
                data ->
                        (type, player) ->
                                new ActionOnTradePower(type, player, data.get("bientity_action"), data.get("bientity_condition"))
        );
    }

    public ActionOnTradePower(PowerType<?> type, LivingEntity entity, Consumer<Pair<Entity, Entity>> action, Predicate<Pair<Entity, Entity>> condition) {
        super(type, entity);
